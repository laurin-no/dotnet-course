...
#warning To protect potentially sensitive information in your connection string, you should move it out of source code. You can avoid scaffolding the connection string by using the Name= syntax to read it from configuration - see https: //go.microsoft.com/fwlink/?linkid=2131148. For more guidance on storing connection strings, see http://go.microsoft.com/fwlink/?LinkId=723263.
                optionsBuilder.UseSqlServer(
                    "Server=tcp:sav-mik-devsql.database.windows.net,1433;Initial Catalog=AdventureWorksLT;Persist Security Info=False;User ID=ets730021s;Password=qMA9m7b4ZGfEgWsM;MultipleActiveResultSets=False;Encrypt=True;TrustServerCertificate=False;Connection Timeout=30;");
            }
        }
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<BuildVersion>(entity =>
            {
                entity.HasKey(e => e.SystemInformationId)
                    .HasName("PK__BuildVer__35E58ECA48EC4566");
        ...
        public Task<int> ProductOrderedQtyAsync(int id)
        {
            var res = SalesOrderDetails
                .Where(s => s.ProductId == id)
                .SumAsync(s => s.OrderQty);

            return res;
        }

        public Task<List<Address>> AddressesInACityAsync(string city, int count)
        {
            var res = Addresses
                .Where(a => a.City == city)
        ...
        public async Task<List<string>> ProductCategoryListAsync()
        {
            ...
            var mainCategories = categories.FindAll(x => x.ParentProductCategory == null);

            var res = mainCategories
                .SelectMany(cat => BuildCategoryList(categories, cat))
                .ToList();

            return res;
        }

        public async Task<CustomerInfo?> GetCustomerInfoAsync(int id, string addressType)
        {
            var info = await Customers
                .FindAsync(id);
            ...
            var addresses = await CustomerAddresses
                .Where(addr => addr.CustomerId == id && addr.AddressType == addressType)
                .Select(addr => addr.Address).ToListAsync();
            ...
            if (addresses.Count <= 0) return res;
        ...
        private List<string> BuildCategoryList(List<ProductCategory> categories, ProductCategory mainCat)
        {
            var res = categories
                .Where(cat => cat.ParentProductCategory == mainCat)
                .Select(cat => $"{cat.ProductCategoryId}: {cat.Name}")
                .Prepend($"--{mainCat.ProductCategoryId}: {mainCat.Name}--")
                .ToList();
